...
- **`index.html` / `how-it-works.html`** — Nav must be updated on both pages to
  add the Install link. `index.html` hero also updated to show the Homebrew
  command.
...
#### - [x] Phase 1.1: Create the install page
...
- [x] The install page opens in a browser without errors
- [x] Three tabs are visible: Homebrew, Debian, GitHub Releases
- [x] Clicking each tab shows only that tab's content
- [x] The active tab is visually distinct from the inactive tabs
- [x] Each tab panel displays the correct install commands
- [x] Copy buttons on each command block copy the command to the clipboard
- [x] The page visually matches the style of the existing site
...
#### - [x] Phase 2.1: Add Install link to top navigation and update homepage hero

The nav on both existing pages gains a new "Install" link pointing to the
install page. The homepage hero install command is also updated from
`go install` to the Homebrew command, directing users to the preferred
package manager install path.

*Technical detail:* [context.md#phase-21](./context.md#phase-21-add-install-link-to-top-navigation-and-update-homepage-hero)

**Acceptance criteria**:

- [x] The Install link appears in the top nav on the homepage
- [x] The Install link appears in the top nav on the How it works page
- [x] Clicking the link navigates to the install page
- [x] The homepage hero shows the Homebrew install command
...
### 2026-04-16 — Phase 1.1: Create the install page

**What was done**: Created `install.html` in the project root with a tabbed
interface (Homebrew, Debian/Ubuntu, GitHub Releases). Appended tab CSS rules
to `assets/css/style.css` and tab-switching JS to `assets/js/main.js` using
the existing `var`/`function` style for consistency.
...
**Discoveries**: The copy-btn class is only applied to the Homebrew
`.install-block` (inline single command). The Debian and GitHub Releases
panels use `.code-block` blocks which are multi-line and don't use copy
buttons — this matches the context.md spec.

### 2026-04-16 — Phase 2.1: Add Install link to top navigation and update homepage hero

**What was done**: Added `<li><a href="install.html">Install</a></li>` to
the nav in both `index.html` and `how-it-works.html`. Updated the hero
install block in `index.html` from the `go install` command to
